# Homework Problem_0804


```python
전체 코드

<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Homework</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
  
</head>
<style>
  .box {
    width : 1200px;
    height : 80px;
    background-color: chocolate;
    color: white;
    font-size: 2rem;
    text-align: left;   
    padding: 15px;
    padding-left : 40px;
  }
  .box2{
    font-size: 2.5rem;
    text-align: center;
    width : 1200px;
    height : 50px;

    

  }
  .container {
    width : 1200px;
    padding : 4px;
  }

  .bd-black{
    border : 1px solid gray;
    padding : 24px;
    border-radius: 4px;
  }

  .sign-in{
    height : 60px;
  }
  .Gitlab{
    font-family: Arial;
    font-size: 1rem;
    font-weight: bold;
  }


  .no2 {
    font-size : 4rem;
    font-weight: bold;
  }

  .Fyp{
    display: inline;
    float: right;
    color: blue;

  }


</style>
<body>
  <div class="d-grid gap-2 col-6 mx-auto mb-5">
    <button class="btn btn-success" type="button">Sign in</button>
  </div>

  <div>
    <nav class="navbar navbar-expand-lg navbar-light bg-dark mb-5">
      <div class="container-fluid">



        <a class="navbar-brand" href="#">
          <img src="img.PNG" alt="" width="50" height="48">
        </a>

        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle text-light" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                프로젝트
              </a>
              <ul class="dropdown-menu bg-dark" aria-labelledby="navbarDropdown">
                <li><a class="dropdown-item text-light" href="#">Action</a></li>
                <li><a class="dropdown-item text-light" href="#">Another action</a></li>
                <li><a class="dropdown-item text-light" href="#">Something else here</a></li>
              </ul>
            </li>

            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle text-light" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                그룹들
              </a>
              <ul class="dropdown-menu bg-dark" aria-labelledby="navbarDropdown">
                <li><a class="dropdown-item text-light" href="#">Action</a></li>
                <li><a class="dropdown-item text-light" href="#">Another action</a></li>
                <li><a class="dropdown-item text-light" href="#">Something else here</a></li>
              </ul>
            </li>

            <li class="nav-item text-success">
              <a class="nav-link text-light" href="#" tabindex="-1" aria-disabled="true">활동</a>
            </li>
            <li class="nav-item">
              <a class="nav-link text-light" href="#" tabindex="-1" aria-disabled="true">마일스톤</a>
            </li>
            <li class="nav-item">
              <a class="nav-link text-light" href="#" tabindex="-1" aria-disabled="true">스니펫</a>
            </li>                        
          </ul>
        </div>
      </div>
    </nav>


  </div>
    
  <div class="Gitlab">
    <nav aria-label="Page navigation example">
      <ul class="pagination">
        <li class="page-item"><a class="page-link text-secondary" href="#">Prev</a></li>
        <li class="page-item active"><a class="page-link text-light" href="#">1</a></li>
        <li class="page-item"><a class="page-link text-dark" href="#">2</a></li>
        <li class="page-item"><a class="page-link text-dark" href="#">3</a></li>
        <li class="page-item"><a class="page-link text-dark" href="#">4</a></li>
        <li class="page-item"><a class="page-link text-dark" href="#">Next</a></li>
      </ul>
    </nav>

   <div class="container">
    <div class="box no1" >Invalid Login or password</div>
    <div class="bg-white no2">SSAFY 전용 GitLab 시스템</div>

    <div><hr class="divider"></div>

    <form class="bd-black mt-4">
      <div class="box2 mb-5">
        sign in
      </div>
      <div><hr class="divider"></div>
      <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">Username or Email</label>
        <input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
      </div>
      <div class="mb-3 mt-4">
        <label for="exampleInputPassword1" class="form-label">Password</label>
        <input type="password" class="form-control" id="exampleInputPassword1">
      </div>
      <div class="mb-3 form-check">
        <input type="checkbox" class="form-check-input" id="exampleCheck1">
        <label class="form-check-label" for="exampleCheck1">Remember me</label>
        <div class="Fyp">
          Forgot your password?
        </div>
      </div>
      <div class="d-grid gap-2 sign-in">
        <button class="btn btn-success" type="button">Sign in</button>
      </div>
      
    </form>
  </div>
  </div>

</body><script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
</html>
```

    1. 박스 내의 문자를 정렬하는 방법에 익숙치 않아서 이전에 배운 padding값을 조절하는 것으로 바꾸었다.
    2. Sign in 밑의 줄이 padding값에 막혀서 끝까지 가지 않는 것을 고쳐야 한다..
